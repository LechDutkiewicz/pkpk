...
<div class="tml tml-profile" id="theme-my-login<?php $template->the_instance(); ?>">
	...
	if ( strlen($error) == 51) : ?>
		...
		<form id="your-profile" action="<?php $template->the_action_url( 'profile', 'login_post' ); ?>" method="post">
			...
			<div class="form-field">
				<label for="first_name"><?php _e( 'Imię', 'theme-my-login' ); ?><span class="asterisk">*</span></label>
				<input type="text" name="first_name" id="first_name" value="<?php echo esc_attr( $profileuser->first_name ); ?>" class="regular-text" required/>
			</div>

			<div class="form-field">
				<label for="last_name"><?php _e( 'Nazwisko', 'theme-my-login' ); ?><span class="asterisk">*</span></label>
				<input type="text" name="last_name" id="last_name" value="<?php echo esc_attr( $profileuser->last_name ); ?>" class="regular-text" required/>
			</div>

			<div class="form-field">
				<label for="email"><?php _e( 'Adres e-mail', 'theme-my-login' ); ?><span class="asterisk">*</span></label>
	...
	<?php if (esc_attr( get_the_author_meta( "accept_privacy", $current_user->ID )) != "yes") : ?>

		<div class="form-field">
			<label for="accept_privacy"><?php the_field( 'ppd_checkbox_title', 'options' ); ?><span class="asterisk">*</span></label>
			<input type="checkbox" name="accept_privacy" class="ios8-switch ios8-switch-lg" id="accept_privacy" value="yes" <?php if (esc_attr( get_the_author_meta( "accept_privacy", $current_user->ID )) == "yes") echo "checked"; ?> required/><label for="accept_privacy"><?php the_field( 'ppd_checkbox_label', 'options' ); ?></label><br />
		</div>
	...
	if ( $show_password_fields ) :
		...
		<div class="form-field form-field--extra-margin">
			<input type="submit" class="btn btn--border-green btn--large" value="<?php esc_attr_e( 'Zaktualizuj profil', 'theme-my-login' ); ?>" name="submit" id="submit" />
		</div>
		<div class="form-field">
...
<p class="tml-submit-wrap">
	...
	<input type="submit" class="btn btn--border-green btn--large" value="<?php esc_attr_e( 'Zmień hasło', 'theme-my-login' ); ?>" name="submit" id="submit" />
</p>
</form>
</div>
